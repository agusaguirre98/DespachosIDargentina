...
export default function useApi() {
  ...
  const API_BASE = useMemo(() => {
    const base = (import.meta.env?.VITE_API_BASE || "http://127.0.0.1:5000").trim();
    ...
    async (path, options = {}) => {
      ...
      if (!ct.includes("application/json")) {
        ...
        const hint = snippet.includes("<!doctype") || snippet.includes("<html")
          ? " (parece HTML: probablemente la SPA en vez del endpoint de API; revisá VITE_API_BASE/proxy/ruta)"
          : "";
